        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2.5">
          ...
          {currentOfficer ? (
            <div className="flex items-center space-x-1.5 text-xs bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 px-3 py-2 rounded-xl font-medium shadow-md">
              ...
              <span className="font-bold text-[11px] truncate max-w-[110px]">{currentOfficer.name}</span>
            </div>
          ) : (
            <button
              onClick={() => setIsLoginModalOpen(true)}
              className="text-xs bg-slate-800 hover:bg-slate-700 text-gray-200 hover:text-white font-bold px-3 py-2 rounded-xl border border-gray-700 transition shadow-md"
            >
              {t.officerLogin}
            </button>
          )}
